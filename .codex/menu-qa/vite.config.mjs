...
export default defineConfig({
  ...
  resolve: { alias: { '@/features/auth/actions': root + 'mocks.tsx', '@': project + 'src', 'next/image': root + 'mocks.tsx', 'next/link': root + 'link.tsx', 'next/navigation': root + 'mocks.tsx' } },
  plugins: [{ name: 'local-menu-actions', enforce: 'pre', resolveId(source, importer) {
    ...
    if (source === './profile-actions' || source === './member-actions') return root + 'mocks.tsx';
  } }],
